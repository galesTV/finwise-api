...
export const getUserCategories = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const doc = await db.collection('user_categories')
      .doc(req.user.uid)
      .get();
...
export const saveUserCategories = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.user?.uid) {
      throw new ApiError(401, 'Usuário não autenticado');
    }
    ...
    // Salva no Firestore
    await db.collection('user_categories')
      .doc(req.user.uid)
...
export const checkSalaryPayment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.user?.uid) {
      throw new ApiError(401, 'Usuário não autenticado');
    }
    ...
    if (!userData || userData.diaRecebimento === undefined || userData.salario === undefined) {
      return res.status(200).json({
        success: true,
        shouldReceive: false,
    ...
    const lastPayment = userData.ultimoRecebimento ? new Date(userData.ultimoRecebimento) : null;
    ...
    const shouldReceive = dayOfMonth === diaRecebimento && 
...
export const updateLastPayment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.user?.uid) {
      throw new ApiError(401, 'Usuário não autenticado');
    }
...
export const updateSubcategoryInUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.user?.uid) {
      throw new ApiError(401, 'Usuário não autenticado');
    }
    ...
    const userDoc = await db.collection('user_categories').doc(req.user.uid).get();
    ...
    const categorias = [...userData.categorias.variavel, ...userData.categorias.fixa];
    ...
    categorias[categoriaIndex].subcategorias = categorias[categoriaIndex].subcategorias.map((sub: any) => 
