...
export const UserController = {
  ...
  async updateCompleteProfile(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.user?.uid) {
        throw new ApiError(401, 'Não autenticado');
      }
  ...
  async validateCompleteProfile(req: Request, res: Response, next: NextFunction) {
    ...
    if (birthDate && !/^\d{2}\/\d{2}\/\d{4}$/.test(birthDate)) {
      return next(new ApiError(400, 'Formato de data inválido. Use DD/MM/AAAA'));
    }
  ...
  async updateSaldo(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.user?.uid) {
        throw new ApiError(401, 'Não autenticado');
      }
  ...
  async getSaldo(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.user?.uid) {
        throw new ApiError(401, 'Não autenticado');
      }
