...
export const createReminder = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const existingReminder = await db.collection('reminders')
...
export const updateReminder = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { title, dueDate, description, priority, isCompleted, category } = req.body;
    ...
    if (typeof isCompleted !== 'undefined' && typeof isCompleted !== 'boolean') {
    ...
    const allowedFields = ['title', 'dueDate', 'description', 'priority', 'isCompleted', 'category'];
    ...
    if (Object.keys(updateData).length <= 1) { // Apenas updatedAt
