...
export const createTransaction = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { type, amount, category, subcategory, date, paid, fixed, reminder, ignore, note, source, wallet } = req.body;
    ...
    next(error instanceof ApiError ? error : new ApiError(500, 'Erro ao criar transação'));
  }
};

export const adicionarValorObjetivo = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.user?.uid) {
      throw new ApiError(401, 'Usuário não autenticado');
    }
    ...
    console.log('🎯 Processando adição de valor ao objetivo:', { objetivoId, valor, userId: req.user.uid });
    ...
    if (Number(valor) > saldoAtual) {
      throw new ApiError(400, `Saldo insuficiente. Saldo atual: R$ ${saldoAtual.toFixed(2)}`);
    }
...
export const searchTransactions = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (category) {
      ...
      console.log(`📊 Após filtro de categoria: ${transactions.length} transações`);
    }
    ...
    if (query && query.trim()) {
      ...
      transactions = transactions.filter(t => {
        ...
        const found = note.includes(searchTerm) ||
    ...
    if (startDate) {
      ...
      console.log(`📊 Após filtro de data inicial: ${transactions.length} transações`);
    }
    ...
    if (endDate) {
      ...
      console.log(`📊 Após filtro de data final: ${transactions.length} transações`);
    }
    ...
    console.log(`✅ Busca concluída: ${transactions.length} transações encontradas`);
