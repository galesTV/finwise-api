...
export const getSummary = async (
  ...
  try {
    ...
    Object.keys(summary.categories).forEach((category) => {
      ...
      const isIncomeCategory = transactions.some(
        (t) => t.category === category && t.type === "income"
      );
      const total = isIncomeCategory
        ? summary.totalIncome
    ...
    next(
      error instanceof ApiError
        ? error
        : new ApiError(500, "Erro ao gerar resumo financeiro")
    );
  }
};
...
export const getMonthlyStats = async (
  ...
  try {
    ...
    if (response.months.length > 0) {
      response.averageIncome =
        response.months.reduce((sum, month) => sum + month.income, 0) /
        response.months.length;
      response.averageExpense =
        response.months.reduce((sum, month) => sum + month.expense, 0) /
        response.months.length;
    ...
    next(
      error instanceof ApiError
        ? error
        : new ApiError(500, "Erro ao gerar estatísticas mensais")
    );
  }
};
